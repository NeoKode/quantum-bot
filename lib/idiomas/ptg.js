...
RpgE: (n='', e='', k='', o='', m='', d='', z='') => [`*[ ! ] Você já realizou uma verificação recentemente, aguarde*\n🕐 ${n}...*\n`, `🪨 ${n ?  '*Você não encontrou recursos por enquanto u.u*' : '*Você descobriu:* '}\n${e ?  `\n🛢️ Gás: ${e}` : ''} ${k ?  `\n⚒️ Wolfram: ${k}` : ''} ${o ?  `\n⚒️ Ouro: ${o}` : ''} ${m ?  `\n⚒️ Ferro: ${m}` : ''} ${d ?  `\n⚒️ Alumínio: ${d}` : ''} ${z ?  `\n⚒️ Silício: ${z}` : ''}\n`, "🗑️ *Nada encontrado até agora*\n\n", `*🏳️ Você encontrou uma nova civilização:* ${n}!, alianças feitas ${e}, agora sua horda é composta por ${k} unidades.`, "🏴 Até agora, nenhum avistamento de vida alienígena foi registrado neste planeta."],
